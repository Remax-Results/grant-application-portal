...
router.get('/org/desc', (req, res) => {
  ...
  router.get('/contact', (req, res) => {
    sqlText = `SELECT  a.id, a.date_received, aq.answer_text AS budget,
                ...
                JOIN review_status AS r ON r.id=a.review_status_id
                JOIN app_question AS aq ON aq.app_id=a.id WHERE aq.question_id=5
                ORDER BY u.contact_name;`;
    pool.query(sqlText)
    .then(result => {res.send(result.rows)})
    .catch(error=> console.log('Error retrieving app table data from server', error))
  }); 

  router.get('/contact/desc', (req, res) => {
    sqlText = `SELECT  a.id, a.date_received, aq.answer_text AS budget,
                u.org_name, u.contact_name, u.phone, u.username, 
                  f.focus, r.status   
                FROM "user" as u
  ...
  router.get('/budget', (req, res) => {
    sqlText = `SELECT  a.id, a.date_received, aq.answer_text AS budget,
                ...
                JOIN review_status AS r ON r.id=a.review_status_id
                JOIN app_question AS aq ON aq.app_id=a.id WHERE aq.question_id=5
                ORDER BY aq.answer_text;`;
    pool.query(sqlText)
    .then(result => {res.send(result.rows)})
    .catch(error=> console.log('Error retrieving app table data from server', error))
  }); 

  router.get('/budget/desc', (req, res) => {
    sqlText = `SELECT  a.id, a.date_received, aq.answer_text AS budget,
                u.org_name, u.contact_name, u.phone, u.username, 
                  f.focus, r.status   
                FROM "user" as u
                ...
                JOIN review_status AS r ON r.id=a.review_status_id
                JOIN app_question AS aq ON aq.app_id=a.id WHERE aq.question_id=5
                ORDER BY aq.answer_text DESC;`;
    pool.query(sqlText)
    .then(result => {res.send(result.rows)})
    .catch(error=> console.log('Error retrieving app table data from server', error))
  }); 

  router.get('/focus/desc', (req, res) => {
    sqlText = `SELECT  a.id, a.date_received, aq.answer_text AS budget,
                u.org_name, u.contact_name, u.phone, u.username, 
                  f.focus, r.status   
                FROM "user" as u
  ...
  router.get('/status/desc', (req, res) => {
    sqlText = `SELECT  a.id, a.date_received, aq.answer_text AS budget,
                u.org_name, u.contact_name, u.phone, u.username, 
                  f.focus, r.status   
                FROM "user" as u

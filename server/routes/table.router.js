...
router.get('/', rejectUnauthenticatedAdmin, (req, res) => {
    const sqlText = `SELECT  a.id, a.date_received, aq.answer_text AS budget,
...
router.get('/:id', rejectUnauthenticatedAdmin, (req, res) => {
    const sqlText= `SELECT  a.id, a.date_received, u.background, u.org_name, u.contact_name, 
...
router.get(`/budget/:id`, rejectUnauthenticatedAdmin, (req, res) => {
    const sqlText = `SELECT answer_text AS budget FROM app_question WHERE app_id=$1 AND question_id=5;`;
    pool.query(sqlText, [req.params.id])
    .then(result => {res.send(result.rows[0])})
    .catch(error=> console.log('Error retrieving app table data from server', error))
});
